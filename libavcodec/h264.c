...
//     close_rate_bins(h->rate_bins_hist);
    stegf = STEGF;
//     bins = 100;
    h->num_stego_features = 2*QP_JUMPS*stegf; // 3
    h->stego_features = (H264FeatureContext**) av_malloc(h->num_stego_features*sizeof(H264FeatureContext*));
    for (j = 0; j < QP_JUMPS; j++) {
      for (i = 0; i < stegf; i++) {
	h->stego_features[i + 2*j*stegf] = init_features("plus_minus", ACCEPT_L, PROB_DELTA*(i+1), QP_OFFSET + j*QP_DELTA);
      }
      for (i = 0; i < stegf; i++) {
	h->stego_features[stegf+i + 2*j*stegf] = init_features("plus_minus", ACCEPT_C, PROB_DELTA*(i+1), QP_OFFSET + j*QP_DELTA);
      }
//       for (i = 0; i < stegf; i++) {
